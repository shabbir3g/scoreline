...
class CPPosts extends WP_Widget {

/*  Constructor
/* ------------------------------------ */
	function CPPosts() {
		parent::__construct( false, 'ClubPress Posts', array('description' => 'Display posts from a category', 'classname' => 'widget_cp_posts') );;	
	}
...
?>
	...
	<ul class="cp-posts group <?php if($instance['posts_thumb']) { echo 'thumbs-enabled'; } ?>">
		...
		<li>
			...
			<div class="post-item-inner group">
				<?php if($instance['posts_category']) { ?><p class="post-item-category"><?php the_category(' &middot; '); ?></p><?php } ?>
...
?>
	...
	.widget .widget-inside .cp-options-posts hr { margin: 20px 0 10px; }
	.widget .widget-inside .cp-options-posts h4 { margin-bottom: 10px; }
	</style>
...
/*  Register widget
/* ------------------------------------ */
if ( ! function_exists( 'cp_register_widget_posts' ) ) {

	function cp_register_widget_posts() { 
		register_widget( 'CPPosts' );
	}
	
}
add_action( 'widgets_init', 'cp_register_widget_posts' );
