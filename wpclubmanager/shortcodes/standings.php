...
$default = array(
	...
	'orderby' => 'pts',
	'order' => 'DESC',
	'linktext' => __( 'View all standings', 'scoreline' ),
	'linkpage' => null,
	'stats' => 'p,w,d,l,f,a,gd,pts',
	'title' => __( 'Standings', 'scoreline' ),
	'thumb' => 1,
	'linkclub' => 0,
);
...
foreach ( $clubs as $club ) {
	...
	if ( $thumb == 1 ) {
		if ( has_post_thumbnail( $club->ID ) ) {
			...
			$club->thumb = '<img src="' . get_template_directory_uri() . '/img/crest-thumb.png" alt="' . get_the_title($club->ID) . '"/>';
		}
	} else {
		$club->thumb = '';
	}
...
if ( $thumb == 1 ) {
	$show_thumb = '<th class="club-thumb"></th>';
} else {
	$show_thumb = '';
}
$output .=
'<div class="cp-table-wrap">
	...
	foreach( $stats as $stat ) {
		$output .= '<th class="cp-stats ' . $stat . '">' . $wpcm_standings_stats_labels[$stat] . '</th>';
	}
	$output .=
			'</tr>
		</thead>
	...
	foreach ( $clubs as $club ) {
		...
		foreach( $stats as $stat ) {
			$output .= '<td class="cp-stats ' . $stat . '">' . $club_stats[$stat] . '</td>';
		}
